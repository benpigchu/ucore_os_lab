...
alloc_proc(void) {
     ...
     */
     ...
     //LAB5 2015011322 : (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2015011322 : (update LAB5 steps)
    /*
     ...
     */
    //LAB8:EXERCISE2 2015011322 HINT:need add some code to init fs in proc_struct, ...
    }
    return proc;
}
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    //LAB8:EXERCISE2 2015011322  HINT:how to copy the fs in parent's proc_struct?
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2015011322  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    for (uint32_t phn=0; phn < elf->e_phnum; phn ++) {
    //    (3.2) read raw data content in file and resolve proghdr based on info in elfhdr
        ret=load_icode_read(fd,ph,sizeof(struct proghdr),elf->e_phoff+(sizeof(struct proghdr))*phn);
        if(ret!=0){
    ...
    for(uint32_t i=0;i<argc;i++){
        uargv[i]=strcpy((char *)(stacktop+argv_offset),kargv[i]);
